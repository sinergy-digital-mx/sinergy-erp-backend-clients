-- Fix payments table schema conflict
-- This script renames the purchase order payments table and recreates the contract payments table

-- Step 1: Rename existing payments table to purchase_order_payments
RENAME TABLE payments TO purchase_order_payments;

-- Step 2: Create the correct payments table for contracts
CREATE TABLE `payments` (
  ...
  CONSTRAINT `FK_payments_tenant` FOREIGN KEY (`tenant_id`) REFERENCES `rbac_tenants` (`id`) ON DELETE CASCADE,
