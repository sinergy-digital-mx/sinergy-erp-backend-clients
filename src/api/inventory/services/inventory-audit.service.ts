...
import {
  ...
  InventoryAuditLineResponseDto,
  InventoryAuditResponseDto,
  InventoryAuditTotalsDto,
  InventoryAuditUserSummaryDto,
  InventoryAuditWarehouseSummaryDto,
...
export class InventoryAuditService {
  ...
  constructor(
    ...
    private readonly lineRepo: Repository<InventoryAuditLine>,
  ...
  async getContext(
    ...
    const totalAvailable = batches.reduce(
      (sum, batch) => sum + this.parseQty(batch.available_quantity),
      0,
    );
  ...
  async create(
    ...
    if (open) {
      throw new BadRequestException(
        `Ya existe una auditoría abierta (${open.folio}) para este almacén${
          dto.product_id ? ' y producto' : ''
        }. Ciérrala o cancélala antes de crear otra.`,
      );
    }

    const includeEmpty = dto.include_empty_lots === true;
    const batches = await this.loadSnapshotBatches(
      tenantId,
      dto.warehouse_id,
      dto.product_id,
      includeEmpty,
    ...
    try {
      const stillOpen = await this.findOpenAudit(
        tenantId,
        dto.warehouse_id,
        dto.product_id,
        qr.manager.getRepository(InventoryAudit),
      );
      if (stillOpen) {
        throw new BadRequestException(
          `Ya existe una auditoría abierta (${stillOpen.folio}) para este alcance`,
        );
      }

      const folio = await this.folioService.generateFolio(tenantId);
      const audit = qr.manager.create(InventoryAudit, {
        id: uuidv4(),
        tenant_id: tenantId,
        folio,
      ...
      await qr.manager.save(InventoryAudit, audit);

      for (const batch of batches) {
        const systemQty = this.roundQty(this.parseQty(batch.available_quantity));
        const line = qr.manager.create(InventoryAuditLine, {
          id: uuidv4(),
      ...
      this.logger.log(
        `Auditoría ${folio} creada: ${batches.length} lotes en almacén ${dto.warehouse_id}`,
      );
  ...
  async updateLines(
    id: string,
    dto: UpdateInventoryAuditLinesDto,
    tenantId: string,
    userId: string,
  ): Promise<InventoryAuditResponseDto> {
    const audit = await this.requireAudit(id, tenantId);
    this.assertStatus(audit, [InventoryAuditStatus.DRAFT], 'capturar conteo');

    const uniqueIds = new Set(dto.lines.map((line) => line.id));
    if (uniqueIds.size !== dto.lines.length) {
      throw new BadRequestException('No se puede repetir la misma línea');
    }

    const existing = await this.lineRepo.find({
      where: { inventory_audit_id: audit.id, id: In([...uniqueIds]) },
    });
    if (existing.length !== uniqueIds.size) {
      throw new NotFoundException('Una o más líneas no pertenecen a esta auditoría');
    }
  ...
  async addLine(
    id: string,
    dto: AddInventoryAuditLineDto,
    tenantId: string,
  ): Promise<InventoryAuditResponseDto> {
    const audit = await this.requireAudit(id, tenantId);
    this.assertStatus(audit, [InventoryAuditStatus.DRAFT], 'agregar lotes');
    ...
    if (already) {
      throw new BadRequestException('Ese lote ya está en esta auditoría');
    }
    ...
    if (batch.warehouse_id !== audit.warehouse_id) {
      throw new BadRequestException('El lote no pertenece al almacén de la auditoría');
    }
    if (audit.product_id && batch.product_id !== audit.product_id) {
      throw new BadRequestException('El lote no corresponde al producto de esta auditoría');
    }
  ...
  async submit(id: string, tenantId: string, userId: string): Promise<InventoryAuditResponseDto> {
    const audit = await this.requireAudit(id, tenantId, true);
    this.assertStatus(audit, [InventoryAuditStatus.DRAFT], 'enviar a autorización');

    const lines = audit.lines ?? [];
    if (lines.length === 0) {
      throw new BadRequestException('La auditoría no tiene líneas para enviar');
    }

    const pending = lines.filter((line) => line.counted_quantity === null);
    if (pending.length > 0) {
      throw new BadRequestException(
        `Faltan ${pending.length} lote(s) por contar antes de enviar a autorización`,
      );
    }
    ...
    if (missingReason.length > 0) {
      throw new BadRequestException(
        `${missingReason.length} lote(s) con diferencia requieren motivo de corrección`,
      );
    }
  ...
  async authorize(
    id: string,
    dto: AuthorizeInventoryAuditDto,
    tenantId: string,
    userId: string,
  ): Promise<InventoryAuditResponseDto> {
    const audit = await this.requireAudit(id, tenantId);
    this.assertStatus(audit, [InventoryAuditStatus.SUBMITTED], 'autorizar');
    ...
    try {
      const locked = await qr.manager
        .createQueryBuilder(InventoryAudit, 'audit')
      ...
      if (locked.status !== InventoryAuditStatus.SUBMITTED) {
        throw new BadRequestException('Solo se puede autorizar una auditoría en revisión');
      }

      const lines = await qr.manager.find(InventoryAuditLine, {
      ...
      for (const line of lines) {
        ...
        if (!batch) {
          throw new NotFoundException(`Lote no encontrado: ${line.inventory_batch_id}`);
        }
        ...
        await qr.manager.save(InventoryBatch, batch);
      ...
      await qr.manager.save(InventoryAudit, locked);
  ...
  async reject(
    id: string,
    dto: RejectInventoryAuditDto,
    tenantId: string,
    userId: string,
  ): Promise<InventoryAuditResponseDto> {
    const audit = await this.requireAudit(id, tenantId);
    this.assertStatus(audit, [InventoryAuditStatus.SUBMITTED], 'rechazar');
  ...
  async cancel(
    ...
    this.assertStatus(
      audit,
      [InventoryAuditStatus.DRAFT, InventoryAuditStatus.SUBMITTED],
      'cancelar',
    );
  ...
  async findAll(
    tenantId: string,
    filters: QueryInventoryAuditDto,
    ...
    if (filters.search) {
      query.andWhere(
        '(LOWER(audit.folio) LIKE LOWER(:search) OR LOWER(product.name) LIKE LOWER(:search) OR LOWER(product.sku) LIKE LOWER(:search))',
    ...
    return {
      data: data.map((audit) =>
        this.mapToResponseDto(audit, [], totalsByAudit.get(audit.id)),
      ),
      total,
      page,
      limit,
  ...
  async findById(id: string, tenantId: string): Promise<InventoryAuditResponseDto> {
  ...
  private async findOpenAudit(
    tenantId: string,
    warehouseId: string,
    productId?: string | null,
    repo: Repository<InventoryAudit> = this.auditRepo,
  ...
  private assertStatus(
    ...
    allowed: InventoryAuditStatus[],
    action: string,
  ): void {
    if (!allowed.includes(audit.status)) {
      throw new BadRequestException(
        `No se puede ${action} una auditoría en estado ${audit.status}`,
      );
    }
  }

  private async loadTotalsMap(ids: string[]): Promise<Map<string, InventoryAuditTotalsDto>> {
    const map = new Map<string, InventoryAuditTotalsDto>();
    if (ids.length === 0) {
      return map;
    }
    ...
    const rows = await this.lineRepo
      ...
      .addSelect(
        'SUM(CASE WHEN line.counted_quantity IS NOT NULL THEN 1 ELSE 0 END)',
        'counted_lines',
      )
      .addSelect(
        `SUM(CASE WHEN line.counted_quantity IS NOT NULL AND ABS(line.variance) >= ${VARIANCE_EPSILON} THEN 1 ELSE 0 END)`,
        'lines_with_variance',
      )
    ...
    for (const row of rows) {
      ...
      map.set(row.audit_id, {
        ...
        total_counted_quantity:
          row.total_counted_quantity === null
            ? null
            : this.formatQty(this.parseQty(row.total_counted_quantity)),
        total_variance:
          row.total_variance === null ? null : this.formatQty(this.parseQty(row.total_variance)),
      });
    }
  ...
  private mapToResponseDto(
    ...
    precomputedTotals?: InventoryAuditTotalsDto,
  ): InventoryAuditResponseDto {
    ...
    return {
      ...
      notes: audit.notes,
      created_by_user: this.mapUser(audit.created_by_user, audit.created_by) as InventoryAuditUserSummaryDto,
  ...
  private mapLine(line: InventoryAuditLine): InventoryAuditLineResponseDto {
    ...
    const systemQty = this.parseQty(line.system_quantity);
    const stockMoved =
      before !== null && Math.abs(before - systemQty) >= VARIANCE_EPSILON;
    ...
    return {
      ...
      quantity_before_post:
        before === null ? null : this.formatQty(before),
      quantity_after_post:
  ...
  private computeTotals(lines: InventoryAuditLineResponseDto[]): InventoryAuditTotalsDto {
    const counted = lines.filter((line) => line.counted_quantity !== null);
    const withVariance = counted.filter(
      (line) => Math.abs(this.parseQty(line.variance)) >= VARIANCE_EPSILON,
    );
    const totalSystem = lines.reduce((sum, line) => sum + this.parseQty(line.system_quantity), 0);
    const totalCounted = counted.reduce(
      (sum, line) => sum + this.parseQty(line.counted_quantity),
      0,
    );
  ...
  ): InventoryAuditUserSummaryDto | null {
    if (!fallbackId) {
      return null;
    }
    return {
