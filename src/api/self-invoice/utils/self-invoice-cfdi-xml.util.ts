import {
  DEFAULT_SAT_CLAVE_PROD_SERV,
  DEFAULT_SAT_CLAVE_UNIDAD,
  SAT_CLAVE_UNIDAD_BY_UOM,
} from '../self-invoice.constants';
...
export function buildSelfInvoiceCfdiXml(input: SelfInvoiceCfdiInput): string {
  ...
  const impuestoBuckets = new Map<string, BuiltTraslado>();
  ...
  for (const line of input.lines) {
    ...
    const share =
      globalDiscount > 0 && lineGrossTotal > 0
    ...
    const claveProd = sanitizeSatClave(line.satClave) || DEFAULT_SAT_CLAVE_PROD_SERV;
    ...
    const trasladoXml = traslados
      .map(
        (traslado) =>
          `          <cfdi:Traslado Base="${formatMoney(traslado.base)}" Impuesto="${traslado.impuesto}" TipoFactor="Tasa" TasaOCuota="${formatRate(traslado.tasa)}" Importe="${formatMoney(traslado.importe)}"/>`,
      )
      .join('\n');
    conceptos.push(
      [
...
function makeTraslado(
  impuesto: BuiltTraslado['impuesto'],
  tasa: number,
  base: number,
): BuiltTraslado {
  return {
...
function resolveClaveUnidad(uomName?: string | null): string {
  const key = String(uomName ?? '')
    .trim()
    .toUpperCase()
    .replace(/\./g, '');
  return SAT_CLAVE_UNIDAD_BY_UOM[key] ?? DEFAULT_SAT_CLAVE_UNIDAD;
}

function sanitizeSatClave(value?: string | null): string {
...
function formatMoney6(value: number): string {
...
export function formatCfdiFecha(date: Date, timeZone = 'America/Tijuana'): string {
