...
export const DEFAULT_INVOICE_EMAIL_SUBJECT =
  'Factura {{invoice_folio}} · {{issuer_name}}';

export const DEFAULT_INVOICE_EMAIL_HTML = `<!DOCTYPE html>
...
export function wrapExtraMessage(message?: string | null): string {
...
export function isLegacyFactoryInvoiceEmailHtml(html?: string | null): boolean {
  if (!html) return true;
  if (!html.includes('Factura electrónica CFDI 4.0')) return false;
