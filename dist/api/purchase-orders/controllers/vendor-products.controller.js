"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VendorProductsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const jwt_auth_guard_1 = require("../../auth/jwt-auth.guard");
const tenant_module_validation_guard_1 = require("../../auth/tenant-module-validation.guard");
const vendor_products_service_1 = require("../services/vendor-products.service");
const query_vendor_products_dto_1 = require("../dto/query-vendor-products.dto");
let VendorProductsController = class VendorProductsController {
    vendorProductsService;
    constructor(vendorProductsService) {
        this.vendorProductsService = vendorProductsService;
    }
    async getVendorProducts(vendorId, query, req) {
        return this.vendorProductsService.getVendorProducts(vendorId, req.user.tenant_id, query);
    }
};
exports.VendorProductsController = VendorProductsController;
__decorate([
    (0, common_1.Get)(':vendorId/products'),
    (0, swagger_1.ApiOperation)({
        summary: 'Catálogo de productos para OC de un proveedor',
        description: 'Por defecto incluye productos activos sin costo (`has_vendor_cost: false`). ' +
            'Usa only_with_cost=true para el comportamiento anterior.',
    }),
    (0, swagger_1.ApiQuery)({ name: 'search', required: false, type: String }),
    (0, swagger_1.ApiQuery)({ name: 'include_without_cost', required: false, type: Boolean }),
    (0, swagger_1.ApiQuery)({ name: 'only_with_cost', required: false, type: Boolean }),
    __param(0, (0, common_1.Param)('vendorId')),
    __param(1, (0, common_1.Query)()),
    __param(2, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, query_vendor_products_dto_1.QueryVendorProductsDto, Object]),
    __metadata("design:returntype", Promise)
], VendorProductsController.prototype, "getVendorProducts", null);
exports.VendorProductsController = VendorProductsController = __decorate([
    (0, swagger_1.ApiTags)('Vendor Products'),
    (0, common_1.Controller)('tenant/vendors'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, tenant_module_validation_guard_1.TenantModuleValidationGuard),
    __metadata("design:paramtypes", [vendor_products_service_1.VendorProductsService])
], VendorProductsController);
//# sourceMappingURL=vendor-products.controller.js.map