...
exports.joinContractFilterRelations = joinContractFilterRelations;
exports.applyContractListFilters = applyContractListFilters;
function joinContractFilterRelations(query, opts) {
    if (opts?.select) {
...
function applyContractListFilters(query, filters) {
    ...
    if (filters.search) {
        query.andWhere('(customer.name LIKE :search OR customer.lastname LIKE :search OR c.contract_number LIKE :search OR property.code LIKE :search OR LOWER(property.cadastral_key) LIKE LOWER(:search))', { search: `%${filters.search}%` });
    }
}
//# sourceMappingURL=contract-list-filters.util.js.map