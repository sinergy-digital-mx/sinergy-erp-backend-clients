...
let InventoryService = InventoryService_1 = class InventoryService {
    ...
    constructor(inventoryBatchRepo, productRepo, transferLineRepo, auditLineRepo, productPriceRepo, productDiscountRepo, productUomRepo, productVendorCostRepo, userRepo, warehouseRepo, fiscalConfigRepo, billingBranchRepo, uomCatalogRepo, s3Service, batchMovementsService) {
        ...
        this.uomCatalogRepo = uomCatalogRepo;
        this.s3Service = s3Service;
        this.batchMovementsService = batchMovementsService;
    }
    async getSignedPhotoUrl(photoKey) {
        if (!photoKey)
            return null;
    ...
    async getStats(tenantId, filters) {
        ...
        const totals = await this.buildStatsBaseQuery(tenantId, filters)
            ...
            .addSelect('SUM(CASE WHEN batch.available_quantity > 0 THEN 1 ELSE 0 END)', 'batches_with_stock')
            .addSelect('COUNT(DISTINCT batch.product_id)', 'total_products')
            .addSelect('COUNT(DISTINCT CASE WHEN batch.available_quantity > 0 THEN batch.product_id END)', 'products_with_stock')
        ...
        const byProductUom = await this.buildStatsBaseQuery(tenantId, filters)
            ...
            .addSelect(`COALESCE(SUM(batch.available_quantity * ${UNIT_COST_SQL}), 0)`, 'po_cost')
          ...
          THEN batch.available_quantity ELSE 0 END), 0)`, 'qty_with_po_cost')
        ...
        for (const row of byProductUom) {
            ...
            const vendorUnitCost = costMaps.byProductUom.get(`${row.product_id}|${row.uom_id}`)
        ...
        return {
            ...
            gross_margin_percentage: this.formatMoney(totalSaleValue > 0 ? (grossMargin / totalSaleValue) * 100 : 0),
    ...
    buildStatsBaseQuery(tenantId, filters) {
        ...
        (0, inventory_location_filter_util_1.applyInventoryLocationFilters)(query, filters);
        return query;
    }
    parseDecimal(value) {
    ...
    async assertLocationHierarchy(tenantId, filters) {
    ...
    async getPosTerminalInventorySummary(tenantId, terminalUserId, filters) {
        ...
        if (!terminalUser) {
            throw new common_1.NotFoundException('Usuario POS no encontrado');
        }
    ...
    async getBranchInventorySummary(tenantId, billingBranchId, filters, options) {
        ...
        if (branchWarehouses.length === 0) {
            throw new common_1.NotFoundException({
                message: options?.emptyWarehousesMessage ??
        ...
        if (selectedWarehouseId && !availableWarehouseIds.has(selectedWarehouseId)) {
            throw new common_1.BadRequestException({
                message: options?.warehouseMismatchMessage ??
        ...
        const matchingProductIds = await this.findBranchSummaryProductIds(tenantId, targetWarehouseIds, filters, skip, limit + 1);
        const hasMore = matchingProductIds.length > limit;
        const pageProductIds = matchingProductIds.slice(0, limit);
        const emptyResponse = {
            billing_branch_id: billingBranchId,
            fiscal_configuration_id: options?.fiscalConfigurationId ?? branch.fiscal_configuration_id ?? null,
            warehouses: branchWarehouses.map((w) => ({
                id: w.id,
                name: w.name,
                status: w.status,
        ...
        const groupsQb = this.buildBranchSummaryBaseQuery(tenantId, targetWarehouseIds, filters)
            ...
            .groupBy('batch.product_id')
            .addGroupBy('batch.uom_id');
        const ranked = (0, product_search_rank_util_1.applyProductSearchOrder)(groupsQb, searchTerm, true);
        if (!ranked) {
            const sortBy = filters.sort_by || 'product_name';
            const sortOrder = (filters.sort_order || 'ASC');
            const sortExpression = sortBy === 'product_sku'
                ? 'MAX(product.sku)'
        ...
        const [batchesByGroup, priceMap, productUomMap, discountsByProductId, photoMap] = await Promise.all([
            this.loadBranchSummaryBatches(tenantId, targetWarehouseIds, groups, filters.only_available),
            this.buildPriceMap(productIds, uomIds),
        ...
        const data = groups.map((row) => {
            ...
            const warehouseNames = Array.from(new Set(batches.map((batch) => batch.warehouse_name).filter(Boolean)));
    ...
    async findBranchSummaryProductIds(tenantId, warehouseIds, filters, skip, fetchLimit) {
        if (warehouseIds.length === 0 || fetchLimit <= 0)
            return [];
        ...
        const ranked = (0, product_search_rank_util_1.applyProductSearchOrder)(qb, filters.search, false);
        if (!ranked) {
            qb.orderBy('product.name', 'ASC');
        }
        const rows = await qb.offset(skip).limit(fetchLimit).getRawMany();
        return rows.map((row) => row.id);
    }
    buildBranchSummaryBaseQuery(tenantId, warehouseIds, filters) {
    ...
    async loadBranchSummaryBatches(tenantId, warehouseIds, groups, onlyAvailable) {
        const map = new Map();
        if (groups.length === 0)
            return map;
        ...
        for (const row of rows) {
            ...
            list.push({
                ...
                ...(0, inventory_measure_util_1.mapBatchMeasure)(row),
    ...
    async buildPriceMap(productIds, uomIds) {
    ...
    async buildProductUomMap(productIds, uomCatalogIds) {
        const map = new Map();
    ...
    async buildDiscountsByProductMap(productIds) {
        const map = new Map();
        if (productIds.length === 0) {
            return map;
        }
        const discounts = await this.productDiscountRepo.find({
            where: { product_id: (0, typeorm_2.In)(productIds), is_active: true },
    ...
    async findAll(tenantId, filters) {
        try {
            this.logger.debug(`Finding all batches for tenant: ${tenantId}, filters: ${JSON.stringify(filters)}`);
            await this.assertLocationHierarchy(tenantId, filters);
            return this.paginateBatches(tenantId, filters);
        }
        catch (error) {
            this.logger.error(`Error finding all batches for tenant ${tenantId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    async findById(id, tenantId) {
        try {
            ...
            (0, inventory_location_filter_util_1.joinInventoryLocation)(batch);
            const found = await batch.getOne();
            if (!found) {
                this.logger.warn(`Batch not found: ${id} for tenant: ${tenantId}`);
                throw new common_1.NotFoundException(`Batch not found: ${id}`);
            }
            ...
            this.logger.error(`Error finding batch ${id} for tenant ${tenantId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    async updateBatch(id, tenantId, dto) {
        ...
        if (!hasTag && !hasMeasureValue && !hasMeasureUom) {
            throw new common_1.BadRequestException('Indica el tag o la medida a actualizar');
        }
        ...
        if (hasMeasureValue || hasMeasureUom) {
            if ((0, inventory_measure_util_1.formatMeasure)(batch.measure) !== null) {
                throw new common_1.BadRequestException('La medida de este lote ya está definida y no se puede cambiar');
            }
            this.assertMeasurePair(dto.measure, dto.measure_uom_id);
            await this.assertMeasureUomExists(dto.measure_uom_id, tenantId);
            batch.measure = dto.measure;
            batch.measure_uom_id = dto.measure_uom_id;
        }
        await this.inventoryBatchRepo.save(batch);
        this.logger.log(`Updated inventory batch ${id}`);
        return this.findById(id, tenantId);
    }
    async listMovements(id, tenantId) {
        return this.batchMovementsService.list(id, tenantId);
    }
    assertMeasurePair(measure, measureUomId) {
        const hasMeasure = measure !== undefined && measure !== null;
        const hasUom = Boolean(measureUomId?.trim());
        if (hasMeasure && !hasUom) {
            throw new common_1.BadRequestException('Indica la unidad del tamaño (Foot, PIES, …). No uses la unidad de la orden de compra');
        }
        if (!hasMeasure && hasUom) {
            throw new common_1.BadRequestException('Indica el tamaño (8, 12, …)');
        }
    }
    async assertMeasureUomExists(measureUomId, tenantId) {
    ...
    async findByPurchaseOrderId(poId, tenantId, filters) {
        try {
            this.logger.debug(`Finding batches for purchase order: ${poId}, tenant: ${tenantId}, filters: ${JSON.stringify(filters)}`);
            return this.paginateBatches(tenantId, filters, {
                extraWhere: (qb) => {
            ...
            this.logger.error(`Error finding batches for purchase order ${poId}, tenant ${tenantId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    calculateTotalQuantity(batches) {
        try {
            ...
            this.logger.debug(`Calculated total quantity: ${totalQuantity} from ${batches.length} batches`);
            return totalQuantity;
        }
        catch (error) {
            this.logger.error(`Error calculating total quantity: ${error.message}`, error.stack);
            throw error;
        }
    }
    mapToResponseDto(batch) {
        return {
            id: batch.id,
    ...
    async paginateBatches(tenantId, filters, options) {
        ...
        this.logger.log(`Successfully retrieved ${data.length} batches out of ${total} total for tenant: ${tenantId}`);
        return {
    ...
    createBatchesQuery(tenantId, filters, options) {
        const qb = this.inventoryBatchRepo
            .createQueryBuilder('batch')
            .where('batch.tenant_id = :tenantId', { tenantId });
        if (options.hydrate) {
    ...
    applyBatchFieldFilters(qb, filters, options) {
    ...
    async getInventorySummary(tenantId, filters) {
        try {
            ...
            const [totalRow, groups] = await Promise.all([
                countQb
                    .select("COUNT(DISTINCT CONCAT(batch.product_id, '|', batch.warehouse_id))", 'total')
                    .getRawOne(),
                groupsQb.getRawMany(),
            ]);
            ...
            this.logger.error(`Error getting inventory summary for tenant ${tenantId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    buildSummaryBaseQuery(tenantId, filters) {
        const qb = this.inventoryBatchRepo
            .createQueryBuilder('batch')
            .innerJoin('batch.product', 'product')
    ...
    async loadSummaryBatches(tenantId, filters, groups) {
        const qb = this.inventoryBatchRepo
            ...
            .addSelect('batch.created_at', 'created_at')
            .where('batch.tenant_id = :tenantId', { tenantId })
            .andWhere(new typeorm_2.Brackets((sub) => {
            groups.forEach((group, index) => {
                sub.orWhere(`(batch.product_id = :pid${index} AND batch.warehouse_id = :wid${index})`, {
    ...
    async signPhotoMap(keys) {
        const unique = [...new Set(keys.filter((key) => !!key))];
        const entries = await Promise.all(unique.map(async (key) => [key, await this.getSignedPhotoUrl(key)]));
        return new Map(entries);
    }
    async loadTransferHistory(batchId) {
    ...
    async loadAuditHistory(batchId) {
        ...
        return lines.map((line) => ({
            ...
            quantity_before_post: line.quantity_before_post === null || line.quantity_before_post === undefined
                ? null
    ...
    mapToDetailResponseDto(batch, transferHistory = [], auditHistory = [], movements = []) {
        ...
        const sales = movements.filter((item) => item.type === inventory_batch_movements_1.INVENTORY_BATCH_MOVEMENT_TYPES.STOCK_SOLD);
        const adjustments = movements.filter((item) => item.type === inventory_batch_movements_1.INVENTORY_BATCH_MOVEMENT_TYPES.INVENTORY_ADJUSTED);
        ...
        return {
            ...
            customs_exchange_rate: this.optionalMoney(batch.purchase_order_batch?.customs_exchange_rate),
            ...
            can_edit_tag: true,
            can_edit_measure: (0, inventory_measure_util_1.formatMeasure)(batch.measure) === null,
    ...
    async attachBatchCosting(dto, batch) {
...
exports.InventoryService = InventoryService = InventoryService_1 = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(inventory_transfer_line_entity_1.InventoryTransferLine)),
    __param(3, (0, typeorm_1.InjectRepository)(inventory_audit_line_entity_1.InventoryAuditLine)),
    __param(4, (0, typeorm_1.InjectRepository)(product_price_entity_1.ProductPrice)),
    __param(5, (0, typeorm_1.InjectRepository)(product_discount_entity_1.ProductDiscount)),
